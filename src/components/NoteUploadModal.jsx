...
const NoteUploadModal = ({ courseId, onClose, onNoteAdded }) => {
  ...
  const handleFileChange = (e) => {
    ...
    if (!isValidType) {
      ...
      setError(
        `Invalid file type. Please select a ${selectedType} file (${expectedTypes})`
      );
      setFile(null);
      setPreview(null);
  ...
  const handleSubmit = async (e) => {
    ...
    if (selectedType === "text" && !writtenText.trim()) {
      // Only check writtenText for text type
    ...
    try {
      ...
      if (selectedType === "text") {
        ...
        fileToUpload = new File([textBlob], fileNameForDb, {
          type: "text/plain",
        });
      }
      ...
      setError(
        error.message || "Failed to upload note. An unknown error occurred."
      );
    } finally {
      setLoading(false);
    }
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
                  ...
                  {!file ? (
                    ...
                    <div className="py-2 space-y-2">
                      ...
                      <div className="flex items-center justify-center text-sm">
                        <span
                          className="text-gray-700 truncate max-w-[200px] font-medium"
                          title={file.name}
                        >
                          {file.name}
                        </span>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200 mt-6">
            ...
            <button
              ...
              disabled={
                loading ||
                (selectedType === "text" ? !writtenText.trim() : !file) ||
                !title.trim()
              }
            >
              {loading ? (
